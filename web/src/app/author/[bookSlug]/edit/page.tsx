    ...
    <main className="relative overflow-hidden">
      <div className="site-shell mx-auto flex min-h-screen flex-col gap-6 px-4 pb-8 pt-28 sm:px-8 sm:pb-12 sm:pt-32">
        <section className="space-y-3">
          ...
          <h1 className="text-[clamp(2rem,5vw,3.2rem)] font-black leading-none tracking-tight">Hikayeni Duzenle</h1>
        ...
        {loadError ? (
          ...
          <>
            <section className="glass-frame grid gap-5 rounded-4xl border border-base-content/12 bg-linear-to-br from-base-100/40 via-base-100/26 to-primary/8 p-5 lg:grid-cols-[260px_minmax(0,1fr)_280px]">
              {/* Sol Sutun */}
                ...
                >
                  ...
                  {coverPreviewUrl || coverImageUrl ? (
                    <div className="relative h-full w-full overflow-hidden rounded-2xl">
                      <Image
                        src={resolveMediaUrl(coverPreviewUrl || coverImageUrl || "")}
                        alt="Kapak"
                        fill
                ...
                <div className="space-y-4">
                  <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]">
                    <label className="flex min-w-0 flex-col gap-1.5">
                      <span className="text-xs font-black uppercase tracking-widest text-base-content/52">Baslik</span>
                    ...
                    <label className="flex min-w-0 flex-col gap-1.5">
                      <span className="text-xs font-black uppercase tracking-widest text-base-content/52">URL Uzantisi (Slug) - Sabit</span>
                  ...
                  <label className="flex min-w-0 flex-col gap-1.5">
                    <p className="text-xs font-black uppercase tracking-widest text-base-content/52">Hakkinda / Ozet</p>
                    <textarea rows={11} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Hikaye ozeti..." className="textarea textarea-bordered min-h-72 w-full rounded-xl border-base-content/12 bg-base-100/20 p-4 text-base leading-relaxed" />
                  </label>

                  <label className="flex min-w-0 flex-col gap-1.5">
                    <p className="mb-1 text-xs font-black uppercase tracking-widest text-base-content/52">Anahtar Kelimeler / Etiketler</p>
                  ...
                  <div className="p-5 rounded-2xl bg-base-content/5 border border-base-content/10">
                    <label className="flex flex-col gap-1.5">
                      <span className="text-xs font-black uppercase tracking-widest text-base-content/40">Eser Tipi (Sabit)</span>
                  ...
                  {bookType === 1 && (
                    <div className={`p-5 rounded-2xl border ${profile?.permissions?.adminAccess ? 'bg-primary/5 border-primary/20' : 'bg-base-100/35 border-base-content/10'}`}>
                      <label className="flex flex-col gap-1.5">
                        <span className={`text-xs font-black uppercase tracking-widest ${profile?.permissions?.adminAccess ? 'text-primary/70' : 'text-base-content/52'}`}>
              ...
              <aside className="flex flex-col rounded-3xl border border-base-content/10 bg-base-100/18 p-4">
                ...
                <div className="flex-1 space-y-2 overflow-y-auto pr-1 custom-scrollbar leading-none max-h-[360px]">
                  {categories.map(c => (
                    <button
                      key={c.id}
                      type="button"
                      onClick={() => toggleCategory(c.id)}
                      className={`flex w-full items-center gap-2 rounded-xl border px-3 py-2 text-sm font-bold transition ${selectedCategories.includes(c.id) ? "border-primary/45 bg-primary/14 text-primary" : "border-base-content/12 bg-base-100/16 text-base-content/82 hover:border-primary/30"
                        }`}
                    >
            ...
            <div className="mt-8 flex flex-col items-center justify-between gap-4 rounded-4xl border border-error/18 bg-error/5 p-6 sm:flex-row">
