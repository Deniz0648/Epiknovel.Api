    ...
    <main className="relative overflow-hidden">
      <div className="site-shell mx-auto flex min-h-screen flex-col gap-6 px-4 pb-8 pt-28 sm:px-8 sm:pb-12 sm:pt-32">
        ...
        <section className="space-y-3">
          ...
          <h1 className="text-[clamp(2rem,5vw,3.2rem)] font-black leading-none tracking-tight">Yeni Hikayene Basla</h1>
              ...
              >
                ...
                {coverPreviewUrl ? (
                  <div className="relative h-full w-full overflow-hidden rounded-2xl">
                    <Image
                      src={resolveMediaUrl(coverPreviewUrl)}
                      alt="Kapak onizleme"
                      fill
                      className={`object-cover transition ${isCoverUploading ? "scale-[1.02] opacity-70" : ""}`}
                    />
                    <div className="absolute inset-x-0 bottom-0 bg-linear-to-t from-black/70 via-black/15 to-transparent px-4 pb-4 pt-10 text-left text-white">
            ...
            <section className="rounded-3xl border border-base-content/10 bg-base-100/18 p-4">
              ...
              <div className="space-y-4">
                <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]">
                  ...
                  <label className="flex min-w-0 flex-col gap-1.5">
                    <span className="text-xs font-black uppercase tracking-widest text-base-content/52">URL Uzantisi (Slug) - Otomatik</span>
                    <input
                      type="text"
                      readOnly
                ...
                <label className="flex min-w-0 flex-col gap-1.5">
                  <span className="text-xs font-black uppercase tracking-widest text-base-content/52">Hakkinda / Ozet</span>
                  <textarea
                    rows={11}
                    value={description}
                    onChange={(e) => setDescription(e.target.value)}
                    placeholder="Hikayenizin kalbini okuyuculara acin..."
                    className="textarea textarea-bordered min-h-72 w-full rounded-xl border-base-content/12 bg-base-100/20"
                  />
                </label>

                <label className="flex min-w-0 flex-col gap-1.5">
                  <span className="text-xs font-black uppercase tracking-widest text-base-content/52">Anahtar Kelimeler / Etiketler</span>
                  <input
                    type="text"
                    value={tags}
                      ...
                      {tagSuggestions.slice(0, 18).map((tagItem) => {
                        ...
                        return (
                          <button
                            ...
                            className={`rounded-full px-3 py-1.5 text-xs font-bold transition ${alreadySelected
                ...
                {profile?.permissions?.adminAccess && (
                  <div className="p-5 rounded-2xl bg-primary/5 border border-primary/20 space-y-4">
                    <label className="flex flex-col gap-1.5">
                      <span className="text-xs font-black uppercase tracking-widest text-primary/70">Yonetici: Eser Tipi</span>
                      <select
                        value={bookType}
                    ...
                    {bookType === 1 && (
                      <label className="flex flex-col gap-1.5 pt-2 animate-in fade-in slide-in-from-top-1 duration-300">
                        <span className="text-xs font-black uppercase tracking-widest text-primary/70">Orijinal Yazar Adi</span>
                        <input
                          type="text"
                ...
                <div className="flex-1 space-y-2 overflow-y-auto pr-1 custom-scrollbar leading-none max-h-[360px]">
                  {categories.map((category) => (
                    <button
                      ...
                      className={`flex w-full items-center gap-2 rounded-xl border px-3 py-2 text-left text-sm font-bold transition ${selectedCategories.includes(category.id)
