...
async function handleProxyRequest(request: NextRequest, method: string) {
  try {
    ...
    // Remove the /api/management/users prefix to get the sub-route for the backend
    const subRoute = pathname.replace("/api/management/users", "") || "/";
    ...
    console.error(`User Management Proxy Error [${method}]:`, error);
    return NextResponse.json(
      { isSuccess: false, message: "Kullanıcı yönetim servisiyle iletişim kurulurken bir hata oluştu." },
      { status: 500 }
    );
  }
}
