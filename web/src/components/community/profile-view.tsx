...
import { followUser, getPublicBooks, unfollowUser, type PublicBookListItem, type PublicUserProfile } from "@/lib/auth";
...
export default function ProfileView({ initialData, slug }: { initialData: PublicUserProfile, slug: string }) {
  ...
  useEffect(() => {
    if (publicProfile.slug && publicProfile.slug !== slug) {
      router.replace(`/community/${publicProfile.slug}`);
    }
  }, [publicProfile.slug, router, slug]);
  ...
  async function handleFollowToggle() {
    ...
    try {
      ...
      setError(toggleError instanceof ApiError ? toggleError.message : "Takip islemi su anda tamamlanamadi.");
    } finally {
  ...
  return (
    <main className="relative overflow-hidden">
      <div className="site-shell mx-auto flex min-h-screen flex-col gap-6 px-4 pb-8 pt-28 sm:px-8 sm:pb-10 sm:pt-32">
        <section className="glass-frame relative overflow-hidden border border-primary/20 bg-linear-to-br from-base-100/92 via-base-100/85 to-primary/8 p-6 sm:p-8">
          <div className="relative z-10">
            <nav className="mb-3 flex items-center gap-1 text-xs font-semibold text-base-content/55">
              <Link href="/" className="transition hover:text-base-content">Anasayfa</Link>
              <ChevronRight className="h-3.5 w-3.5 text-base-content/35" />
              <Link href="/community" className="transition hover:text-base-content">Topluluk</Link>
              <ChevronRight className="h-3.5 w-3.5 text-base-content/35" />
            ...
            <h1 className="mt-2 text-[clamp(1.9rem,4vw,3rem)] font-black leading-[0.95]">{publicProfile.displayName}</h1>
          </div>
        </section>
